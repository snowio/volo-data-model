...
final class Order
{
    ...
    {
        $result = self::of($json['espOrderNo']);
        $result->orderType = $json['orderType'] ?? null;
        $result->orderSource = $json['orderSource'] ?? null;
        $result->invoiceNumber = $json['invoiceNumber'] ?? null;
        $result->externalReference = $json['externalReference'] ?? null;
        ...
        $result->customerFax = $json['customerFax'] ?? null;
        ...
        $result->insurance = $json['insurance'] ?? null;
        $result->discount = $json['discount'] ?? null;
        $result->voucherCode = $json['voucherCode'] ?? null;
        $result->orderTotal = $json['orderTotal'] ?? null;
        $result->paymentComplete = $json['paymentComplete'] ?? null;
        $result->currencyCode = $json['currencyCode'] ?? null;
        $result->exchangeRate = $json['exchangeRate'] ?? null;
        $result->sellerUsername = $json['sellerUsername'] ?? null;
        $result->sellerId = $json['sellerId'] ?? null;
        $result->courierProfileName = $json['courierProfileName'] ?? null;
        $result->buyerId = $json['buyerId'] ?? null;
        $result->storeId = $json['storeId'] ?? null;
        $result->shipToStore = $json['shipToStore'] ?? null;
        $result->orderStatus = $json['orderStatus'] ?? null;
        $result->flag1 = $json['flag1'] ?? null;
        $result->flag2 = $json['flag2'] ?? null;
        $result->courierName = $json['courierName'] ?? null;
        $result->courierService = $json['courierService'] ?? null;
        $result->courierServiceCode = $json['courierServiceCode'] ?? null;
        $result->fulfilmentType = $json['fulfilmentType'] ?? null;
        $result->webOrderID = $json['webOrderID'] ?? null;
        $result->shippingDate = $json['shippingDate'] ?? null;
        $result->invoiceDate = $json['invoiceDate'] ?? null;
        $result->tradeSale = $json['tradeSale'] ?? null;
        $result->courierTracking = $json['courierTracking'] ?? null;
        $result->actualCourierCost = $json['actualCourierCost'] ?? null;
        $result->orderItems = ItemSet::fromJson($json['orderItems'] ?? []);
        $result->orderCredits = OrderCreditSet::fromJson($json['orderCredits'] ?? []);
        $result->payments = PaymentSet::fromJson($json['payments'] ?? []);
        return $result;
    }
